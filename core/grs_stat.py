...
class Gas:
    def __init__(self, composition, temperature, pressure, rate=0):
        ...
        self.actual_density = CP.PropsSI('D', 'T', self.temperature, 'P', self.pressure, self.mixture)
        self.normal_density = CP.PropsSI('D', 'T', 273.15, 'P', 101325, self.mixture)
        self.standard_density = CP.PropsSI('D', 'T', 293.15, 'P', 101325, self.mixture)
        self.mass_flow = self.standard_density * self.rate
        self.actual_rate = self.mass_flow / self.actual_density
        self.molecular_weights = {'Methane': 16.04, 'Ethane': 30.07, 'Propane': 44.09, 'Isobutane': 58.12,
                                  'Butane': 58.12, 'Isopentane': 75.15,
                                  'Pentane': 75.15, 'Hexane': 86.18, 'Oxygen': 31.99806, 'Nitrogen': 28.01286,
                                  'CarbonDioxide': 44.01}
        self.component_specific_heat = {key: (CP.PropsSI('C', 'T', self.temperature, 'P', self.pressure, key)) for key
                                        in self.composition}
        ...
        self.mass_fraction = {key: (composition[key] * self.molecular_weights[key] / total_molecular_weight) for key in
                              self.composition}
        try:
    ...
    def get_actual_rate(self, temperature, pressure):
        ...
        return self.mass_flow / CP.PropsSI('D', 'T', temperature, 'P', pressure, self.mixture)
